...
class BackendTupfile(object):
    ...
    def gen_sources_rules(self, extra_inputs):
        compilers = [
            # ('.S', 'AS', 'ASFLAGS'), # TODO: need bug 1416059 and maybe ted
            # patch, or set $(CC) in Tuprules.tup
            ('.cpp', 'CXX', 'CXXFLAGS'),
            ('.c', 'CC', 'CFLAGS'),
        ]
        for extension, compiler, flags in compilers:
            srcs = sorted([f for f in self.sources if f.endswith(extension)])
            for src in srcs:
                cmd = [self.environment.substs[compiler]]
...
class TupOnly(CommonBackend, PartialBackend):
    ...
    def consume_object(self, obj):
        ...
        if isinstance(obj, GeneratedFile):
            ...
            if obj.relobjdir.startswith('xpcom'):
                backend_file.sources.extend(obj.files)
    ...
    def _process_unified_sources(self, obj):
        backend_file = self._get_backend_file_for(obj)
        if obj.relobjdir.startswith('xpcom'):
            backend_file.sources.extend([f[0] for f in obj.unified_source_mapping])
